...
and backend pack, enforce the 12-file/60k-character task budget, and reuse verified
nodes by declared input hash. The default lifecycle is sequential: approved HTML,
frontend, backend, integration, testing, then delivery. Parallel work is allowed only
inside a phase when explicitly planned and path leases do not overlap.
